...
| **`semaphore-integration`** | Anything Semaphore-adjacent that isn't taxonomy authoring or tuning: whether Semaphore is reachable at all (`semaphore_status` / `semaphore_studio_status` first, `SEMAPHORE_*` vars as *server* config), then wiring it to MarkLogic — ingest-time classification vs reprocess enrichment vs REST transform vs DHF; the enrichment module; facet indexes. | 1 reference: enrichment module |
