...
Every registered tool, by group, is listed in `references/tool-index.md`. The table
above maps problems to the right starting point; open the index only when you need to
check whether a capability exists at all — for example, before telling a user a feature
isn't available, or before adding a new tool to confirm nothing already covers it.
